...
// Event listener: Client selection
...
// Event listener: Assign VM button in the VM column
    ...
    get(vmRef)
        .then((snapshot) => {
            if (snapshot.exists() && snapshot.val()) {
                // Refresh the VM column then alert if already occupied
                loadVMs(selectedClient);
...
// Event listener: Unassign VM button in Active Sessions column
...
// Event listener: Refresh Active Sessions button (with 2-sec timeout)
